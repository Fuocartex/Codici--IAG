...
// MioFile.h
	...
	int** edge_Matrix(SimplicialComplex*, int, int);
    ...
    #pragma region edge_Matrix
		...
		int** edge_Matrix(SimplicialComplex* sc, int size, int n) {
			int** matrix = (int**)calloc(sc[n].size, sizeof(int*));
			...
			for(int i = 0; i < sc[n].size; i++) {
				matrix[i] = (int*)calloc(sc[n - 1].size, sizeof(int));
