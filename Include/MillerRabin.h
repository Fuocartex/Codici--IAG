// identico a MillerRabinGMP.c di Elena, ho solo copiato le funzioni in un .h in modo da poterle includere
// all'interno di altri codici
...
// Funzione ausiliaria per verificare se n � composto
int isComposite(const mpz_t n, const mpz_t a, const mpz_t d, long h) {
    ...
    for (long i = 1; i < h; i++) {
        ...
        // Se x == 1 e prev != n - 1, allora � composto
        if (mpz_cmp_ui(x, 1) == 0) {
    ...
    // Se dopo tutte le iterazioni x ? 1, allora � sicuramente composto
    if (mpz_cmp_ui(x, 1) != 0) {
        mpz_clears(x, prev, temp, NULL);
        return 1;
    }
